	...
	<section>
		...
		$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
		 query_posts("category_name=acordaos&posts_per_page=20&paged=".$paged);

		 ?>
		<div class="principalArtigo artigo row">
			<?php
				    // Get the ID of a given category
				   	$category_acordaos = get_category_by_slug("acordaos")->cat_ID;

				    // Get the URL of this category
 				   $category_link_acordaos = get_category_link( $category_acordaos );

 				   $category_link_acordaosPage = get_page_link(19);
			?>
