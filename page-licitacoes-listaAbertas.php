	<?php /*Template Name: Licitações - listar cat licitacao_abertas*/?>
	<?php get_header(); ?>
	...
	<section>
		...
		$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
		 query_posts("category_name=licitacao_abertas&posts_per_page=20&paged=".$paged);

		 ?>
		<div class="principalArtigo artigo row">
			<?php
				    // Get the ID of a given category
				   	$category_licitacao_abertas = get_category_by_slug("licitacao_abertas")->cat_ID;

				    // Get the URL of this category
 				   $category_link_licitacao_abertas = get_category_link( $category_licitacao_abertas );

 				   $category_link_licitacao_abertasPage = get_page_link(19);
			?>
			...
			<article>
				<div class="listaEventos">
					...
					<?php if (have_posts()) :?>
						...
						<?php endwhile; else: ?>
							<p><?php _e( 'Não existem licitações licitacao_abertas no momento.' ); ?></p>
					<?php endif; ?>
					</ul>
				</div>
			</article>
