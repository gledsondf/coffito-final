	<?php /*Template Name: Eleições - listar eleições*/?>
	<?php get_header(); ?>
	...
	<section>
		...
		$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
		 query_posts("category_name=eleicoes&posts_per_page=20&paged=".$paged);

		 ?>
		<div class="principalArtigo artigo row">
			<?php
				    // Get the ID of a given category
				   	$category_eleicoes = get_category_by_slug("eleicoes")->cat_ID;

				    // Get the URL of this category
 				   $category_link_eleicoes = get_category_link( $category_eleicoes );

 				   $category_link_eleicoesPage = get_page_link(19);
			?>
			...
			<article>
				<div class="listaEventos">
					...
					<?php if (have_posts()) :?>
						...
						<?php endwhile; else: ?>
							<p><?php _e( 'Não existem eleicões no momento.' ); ?></p>
					<?php endif; ?>
					</ul>
				</div>
			</article>
